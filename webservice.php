<?php
  ...
  if($APP_PAYMENT_METHOD == "Braintree"){
    ...
    // $myfile = file_put_contents('REQUEST.txt', PHP_EOL.'----------------------------------'.PHP_EOL.json_encode($_REQUEST) , FILE_APPEND | LOCK_EX);	
	...
	if($type=="checkPassenger"){
		$data = getPassengerDetailInfo($_REQUEST['id']);
		echo json_encode($data);
	}
	
	
	function getPassengerDetailInfo($passengerID,$cityName=''){
		global $generalobj,$obj,$demo_site_msg,$PHOTO_UPLOAD_SERVICE_ENABLE,$parent_ufx_catid,$generalConfigArr,$tconfig,$vTimeZone,$vUserDeviceCountry,$_REQUEST;
		...
		$myfile = file_put_contents('example1.txt', json_encode($row).PHP_EOL , FILE_APPEND | LOCK_EX);
      ...
      $row[0]['user_available_balance'] = strval($user_available_balance);
		...
		$user_available_balance_value = $generalobj->get_user_available_balance_app_display($passengerID,"Rider",'Yes');
      	...
      	if($user_available_balance_value <= 0){
			...
			$sql_currency = "SELECT fWalletAmount1,fWalletAmount2,fWalletAmount3 from currency WHERE vName = '".$vCurrencyPassenger."'";
			$user_wallet_Key = $obj->MySQLSelect($sql_currency);
			$row[0]['WALLET_FIXED_AMOUNT_1']= $user_wallet_Key[0]['fWalletAmount1'];
			$row[0]['WALLET_FIXED_AMOUNT_2']= $user_wallet_Key[0]['fWalletAmount2'];
			$row[0]['WALLET_FIXED_AMOUNT_3']= $user_wallet_Key[0]['fWalletAmount3'];
      ...
      if($APP_TYPE = "Ride-Delivery-UberX"){
        ...
        $row[0]['RIDE_SHOW_SELECTION'] = "None";
		$row[0]['MOTO_RIDE_SHOW_SELECTION'] = "None";
			/* fetch value */
			return $row[0];
    ...
    ChangeDriverVehicleRideDeliveryFeatureDisable($driverId);
      ...
      $Data[0]['user_available_balance'] = strval($user_available_balance);
			...
			$sql_currency = "SELECT fWalletAmount1,fWalletAmount2,fWalletAmount3 from currency WHERE vName = '".$Data[0]['vCurrencyDriver']."'";
			$user_wallet_Key = $obj->MySQLSelect($sql_currency);
			$Data[0]['WALLET_FIXED_AMOUNT_1']= $user_wallet_Key[0]['fWalletAmount1'];
			$Data[0]['WALLET_FIXED_AMOUNT_2']= $user_wallet_Key[0]['fWalletAmount2'];
			$Data[0]['WALLET_FIXED_AMOUNT_3']= $user_wallet_Key[0]['fWalletAmount3'];
	...
	if($type=="signup"){
		...
		if($imei !== ''){
            ...
            // # Check for Blocked Users #
				$sql_blocked = "SELECT block_id,iUserId,imei,vName,vLastname,status,vType FROM `user_blocklist` WHERE imei='".$imei."' ORDER BY block_id DESC LIMIT 0,1";
				$res_blocked = $obj->MySQLSelect($sql_blocked);
				if(sizeof($res_blocked)){
				    if($res_blocked[0]['status']==='ACTIVE'){
			        	$returnArr['Action']="0";
			            $returnArr['message'] ="LBL_IMEI_BLOCKED";
				        echo json_encode($returnArr);exit;
				    }
				}
			// # Check for Blocked Users #
...
/*					$generalobj->send_email_user("DRIVER_REGISTRATION_ADMIN",$maildata); */
	...
	if ($type == "loadAvailableCab") {
		...
		$myfile = file_put_contents('ScheduleRide.txt', PHP_EOL.'----------------------------------'.PHP_EOL.json_encode($_REQUEST) , FILE_APPEND | LOCK_EX);	
    ...
    if($eShowOnlyMoto == "" || $eShowOnlyMoto == NULL){
      $eShowOnlyMoto = "No";
		...
		$DataArr = getOnlineDriverArr($passengerLat,$passengerLon,$address_data,"No","No",$Check_Driver_UFX,$Check_Date_Time,"","",$eType,$iVehicleTypeId);
	...
	// applying check if eType is not uberX the only fetch the vehicle types
    if ($eType != "UberX") {
    ...
    if($eRental == "Yes"){
      ...
      $vehicleTypes = array_values($vehicleTypes_New); 
 	  ...
 	  //  $myfile = file_put_contents('DriversList1.txt', json_encode($returnArr) , FILE_APPEND | LOCK_EX);	
	...
	if($type=="getDriverStates"){
		...
		if($APP_TYPE == "berX" || $APP_TYPE == "Ride-Delivery-erX"){
    ...
    // ###########################################################################
    // Promo code Check for ONUX if multiple accounts are registered on same device
    // ###########################################################################
// 		if($promoCode === "ONUX"){
//     		$sql2 = "SELECT imei FROM register_user where eStatus = 'Active' AND iUserId = ".$iUserId;
//     		$imei = $obj->MySQLSelect($sql2);
    		
//     		$sql3 = "SELECT count(iUserId) as accounts FROM register_user where eStatus = 'Active' AND imei = '".$imei[0]['imei']."'";
//     		$data3 = $obj->MySQLSelect($sql3);
// 		    if(count($data3) && $data3[0]['accounts'] > 1){
...
// 		}
		...
		if($promoCode === "ONUX"){
    		...
    		$sql3 = "select iTripId from trips where vCouponCode = 'ONUX'and iActive = 'Finished' and iUserId IN ( SELECT iUserId FROM register_user where eStatus = 'Active' AND imei = ".$imei[0]['imei'].")";
    		$data3 = $obj->MySQLSelect($sql3);
    		
    		if(count($data3)>0){
    		  //  $myfile = file_put_contents('Coupon.txt', PHP_EOL.'----------------------------------'.PHP_EOL.json_encode(count($data3)) , FILE_APPEND | LOCK_EX);	    		
    		    $returnArr['Action']="0";// code is invalid
    		...
    		$sql4 = "select iTripId from trips where vCouponCode = '$promoCode' and iActive = 'Finished' and iUserId IN ( SELECT iUserId FROM register_user where eStatus = 'Active' AND imei = ".$imei[0]['imei'].")";
    		$data4 = $obj->MySQLSelect($sql4);
	...
	if($type=='estimateFareNew'){
		...
		$myfile = file_put_contents('FARE.txt', PHP_EOL.'----------------------------------'.PHP_EOL.json_encode($_REQUEST) , FILE_APPEND | LOCK_EX);	
		$seconds = $time;
		...
		$Fare_data=calculateFareEstimateAll($time,$seconds,$distance,$SelectedCar,$iUserId,1,"","",$PromoCode,1,0,0,0,"","Passenger",$iQty,$SelectedCarTypeID,$isDestinationAdded,$eFlatTrip,$fFlatTripPrice,$sourceLocationArr,$destinationLocationArr);
...
//	}
	...
	if($type=='getEstimateFareDetailsArr'){
		...
		$Fare_data=calculateFareEstimateAll($time,$seconds,$distance,$SelectedCar,$iUserId,1,"","",$promoCode,1,0,0,0,"DisplySingleVehicleFare",$userType,1,"",$isDestinationAdded,$eFlatTrip,$fFlatTripPrice,$sourceLocationArr,$destinationLocationArr);
...
// 		$myfile = file_put_contents('FARE.txt', PHP_EOL.'----------------------------------'.PHP_EOL.json_encode($Fare_data) , FILE_APPEND | LOCK_EX);	
		$returnArr["Action"] = "1";
		$returnArr["message"] = $Fare_data;
		echo json_encode($returnArr);
	}
...
// 		$reschedule = isset($_REQUEST["Reschedule"]) ? $_REQUEST["Reschedule"] : '';        
		/*added for rental*/
...
Gracias!";
    ...
    $eWalletAdjustment = get_value('register_user', 'eWalletAdjustment', 'iUserId', $iUserId, '', 'true');
       ...
       if($vTripStatus == "Canceled" || $vTripStatus == "Cancelled" || $vTripStatus == "Finished" || $vTripStatus == "On Going Trip"){
		...
		$pickUpLatitude = $tripCancelData[0]['tStartLat'];
		$destLatitude = $tripCancelData[0]['tEndLat'];
		$pickUpLongitude = $tripCancelData[0]['tStartLong'];
		$destLongitude = $tripCancelData[0]['tEndLong'];
		...
		$onlineDrivers = "";
		$onlineDriversArr = getOnlineDriverArr($pickUpLatitude,$pickUpLongitude,$address_data,$DropOff,"No","No","",$destLatitude,$destLongitude,$eType,'');
		$data_drv = $onlineDriversArr['DriverList'];
		for($i=0; $i<sizeof($data_drv) ; $i++){
		    $onlineDrivers .= $data_drv[$i]['iDriverId'];
		    if($i < sizeof($data_drv)-1){
		        $onlineDrivers .=  ",";
		    }
		  //  array_push($onlineDrivers,$driver['iDriverId']);
		}
		
		
// 		$myfile = file_put_contents('SendRequest1.txt', PHP_EOL.'----------------------------------'.PHP_EOL.json_encode($onlineDrivers) , FILE_APPEND | LOCK_EX);	
    ...
    $eCancelChargeFailed = "No";
      ...
      if($fCancellationFare > 0 && $eWalletAdjustment == "Yes"){
		...
		$message_arr['DriversList'] = $onlineDrivers;
		...
		$pendingTrip = get_value('register_driver', 'pendingTripId', 'iDriverId',$iDriverId,'','true');
		$myfile = file_put_contents('PendingTrip.txt', PHP_EOL. $pendingTrip , FILE_APPEND | LOCK_EX);

		if($pendingTrip != NULL && $pendingTrip != ''){
		    $myfile = file_put_contents('PendingTrip.txt', PHP_EOL. 'AAYA' , FILE_APPEND | LOCK_EX);
			$Data_update_driver['vTripStatus']='On Going Trip';
			$Data_update_driver['pendingTripId']=NULL;	
		}else{
	...
	if($type == "GenerateTrip"){
      ...
      if($eType == "UberX"){
		...
		$iCabBookingId= isset($_REQUEST["iCabBookingId"]) ? $_REQUEST["iCabBookingId"] : '';
		$vMsgCode= isset($_REQUEST["vMsgCode"]) ? $_REQUEST["vMsgCode"] : '';
		$pending= isset($_REQUEST["pending"]) ? $_REQUEST["pending"] : 'Yes';
		$setCron= isset($_REQUEST["setCron"]) ? $_REQUEST["setCron"] : 'No';
		//$APP_TYPE = $generalobj->getConfigurations("configurations","APP_TYPE");
		
		################ Comment For ongoing trip ########################
        // 		$sqldata = "SELECT iTripId FROM `trips` WHERE ( iActive='On Going Trip' OR iActive='Active' ) AND iDriverId='" .$driver_id. "'";
        // 		$TripData = $obj->MySQLSelect($sqldata);
        // 		if(count($TripData) > 0){
        // 			$returnArr['Action'] = "0";
...
// 			$trip_status = "Active";
			...
			$where = " iUserId = '$passenger_id'";
			$Data_update_passenger['iTripId']=$iTripId;
			$Data_update_passenger['vTripStatus']='Active';
			$id = $obj->MySQLQueryPerform("register_user",$Data_update_passenger,'update',$where);
...
// 			$Data_update_driver['vTripStatus']=$trip_status;
			$Data_update_driver['vRideCountry']=$vRideCountry;
			$Data_update_driver['vAvailability']="Not Available";
			$Data_update_driver['pendingTripId']=$iTripId;
			$id = $obj->MySQLQueryPerform("register_driver",$Data_update_driver,'update',$where);
	...
	if ($type == "updateDriverLocations") {
		...
		$longitude_driver = isset($_REQUEST["longitude"]) ? $_REQUEST["longitude"] : '';
		$vTimeZone = isset($_REQUEST["vTimeZone"]) ? $_REQUEST["vTimeZone"] : '';
// 		$myfile = file_put_contents('DriverLocations.txt', PHP_EOL. json_encode($_REQUEST) , FILE_APPEND | LOCK_EX);
        ...
        $Data_update_epayment['ePaymentCollect']='Yes';
          ...
          $id1 = $obj->MySQLQueryPerform("trips",$Data_update_epayment,'update',$where);
			...
			try{
			    ...
			    if($error3=="Your card does not support this type of purchase.")
				    $error3="Su tarjeta no admite este tipo de compra.";
			//	var_dump($result1);
...
//	$returnArr['message']=$statusofchargeid;
	...
	if($type == "ProcessEndTrip"){
		...
		$query1 = "SELECT * FROM user_wallet WHERE iUserId = $userId";
        $wallet = $obj->MySQLSelect($query1);
	
	    $myfile = file_put_contents('iFare.txt','In the Start '.PHP_EOL. json_encode($wallet).PHP_EOL , FILE_APPEND | LOCK_EX);
...
// 		 $myfile = file_put_contents('example1.txt', $txt.PHP_EOL , FILE_APPEND | LOCK_EX);
    ...
    ## Update User Outstanding Amount ##
		...
		if($id>0){
		    ...
		    $myfile = file_put_contents('iFare.txt','In the end '.PHP_EOL. json_encode($wallet).PHP_EOL , FILE_APPEND | LOCK_EX);
	...
	if($type=="CollectPayment"){
    	...
    	$txt=$ePaymentCollect."|".$iFare."|".$estimatefare."|".$isCollectCash;
		$myfile = file_put_contents('iFare.txt', $txt.PHP_EOL , FILE_APPEND | LOCK_EX);
		   // $myfile = fopen("example.txt", "w") or die("Unable to open file!");
		...
		//	fwrite($myfile, "enter into payment condition \n");
			$vStripeCusId = get_value('register_user', 'vStripeCusId', 'iUserId', $iUserId,'','true');
		$vCardVerified = get_value('register_user', 'vCardVerified', 'iUserId', $iUserId,'','true');
		if($vCardVerified != 1){
		    $returnArr['Action'] = "0";
			$returnArr['message']="LBL_UNVERIFIED_CARD";
		    echo json_encode($returnArr);
		    exit();
		}
            $vBrainTreeToken = get_value('register_user', 'vBrainTreeToken', 'iUserId', $iUserId,'','true');
     ...
     // fwrite($myfile, "charge \n");
      ...
      $status = $ChargeidArr['status']; 
		...
		if($id >0){
		    ...
		    $driverfare=$tripData[0]['fTripGenerateFare']*0.15;
		   ...
		   if($walletdabet>0){
		    ...
		    $driver_date = @date("Y-m-d H:i:s");
	        $driverfare=$walletdabet*0.15;
	        $driverfare=$walletdabet-$driverfare;
	       
            $generalobj->InsertIntoUserWallet($driverId, "Driver",$driverfare,"CREDIT",$iTripId, "Deposit", "#LBL_CREDITED_BOOKING#".$tripData[0]['vRideNo'], "Unsettelled", $driver_date);
		   }else{
		       // Deduct comission when driver recieved cash from from rider.
		       $driverfare=$tripData[0]['fTripGenerateFare']*0.85;
		       $driverfare=$tripData[0]['fTripGenerateFare']-$driverfare;
		        $driver_date = @date("Y-m-d H:i:s");
                $generalobj->InsertIntoUserWallet($driverId, "Driver", $driverfare,"DEBIT",$iTripId, "Comission", "#LBL_DEBITED_BOOKING#".$tripData[0]['vRideNo'], "Unsettelled", $driver_date);
		   }
		}
			$returnArr['Action'] = "1";
	...
	if($type=="addMoneyUserWallet"){
		...
		$vCardVerified = get_value('register_user', 'vCardVerified', 'iUserId', $iMemberId,'','true');
		if($vCardVerified != 1){
		    $returnArr['Action'] = "0";
			$returnArr['message']="LBL_UNVERIFIED_CARD";
		    echo json_encode($returnArr);
...
// 		$myfile = file_put_contents('COUPON.txt', json_encode($_REQUEST) , FILE_APPEND | LOCK_EX);	
		...
		* sin( radians( vLatitude ) ) ) ),2) AS distance, register_user.*  FROM `register_user`
		WHERE (vLatitude != '' AND vLongitude != '' AND eStatus='Active' AND tLastOnline >  DATE_SUB(NOW(), INTERVAL 20 DAY))
		...
		foreach ($Data as $value) {
			$dataofuser = getLngLat("Online",$value['vLongitude'],$value['vLatitude'],$value['iUserId']);
		...
		if(SITE_TYPE == 'Demo'){
			...
			* sin( radians( tStartLat ) ) ) ),2) AS distance, trips.*  FROM `trips`
			WHERE (tStartLat != '' AND tStartLong != '' AND tTripRequestDate >= DATE_SUB(CURDATE(), INTERVAL 20 DAY))
			...
			* cos( radians( tStartLong ) - radians(".$sourceLon.") )
			+ sin( radians(".$sourceLat.") )
			* sin( radians( tStartLat ) ) ) ),2) AS distance, trips.*  FROM `trips`
			WHERE (tStartLat != '' AND tStartLong != '' AND tTripRequestDate >= DATE_SUB(NOW(), INTERVAL 20 DAY))
			HAVING distance < ".$radius." ORDER BY `trips`.iTripId DESC";
		}
		...
		foreach ($Dataoftrips as $value1) {
			$valuetrip = getLngLat("History",$value1['tStartLong'],$value1['tStartLat'],$value1['iTripId']);
		...
		$features = array_merge($storeuser,$storetrip);
		$newArr->type = "FeatureCollection";
        $newArr->features = $features;
        
		if(sizeof($features)>0){
			$returnData['Action']="1";
			$returnData['message']=$newArr;
			}else{
			$returnData['Action']="0";
		}
		echo json_encode($returnData);
	...
	if ($type == "uploaddrivedocument") {
		...
		if($doc_file != ""){
			...
			// Write code to compress image 
			$ext = pathinfo($image_name, PATHINFO_EXTENSION);
// 			$myfile = file_put_contents('COUPON.txt', PHP_EOL.'----------------------------------'.PHP_EOL.json_encode($ext) , FILE_APPEND | LOCK_EX);	
			
		    if($ext == 'jpg' || $ext == 'png' || $ext == 'jpeg'){
			$maxDim = 1000;
			list($width, $height, $type, $attr) = getimagesize( $image_object );
            if ( $width > $maxDim || $height > $maxDim ) {
                $target_filename = $image_object;
                $ratio = $width/$height;
                if( $ratio > 1) {
                    $new_width = $maxDim;
                ...
                $src = imagecreatefromstring( file_get_contents( $image_object ) );
	...
	########################################################################################
	// Add Trip to Driver's pending trip, when he is on another trip ending in 2 minutes
	########################################################################################
	
		
	if($type=="UpdateDriverAvailability"){
		
		$Did              = isset($_REQUEST["DriverAutoId"]) ? $_REQUEST["DriverAutoId"] : '';
		$availabilityStatus              = isset($_REQUEST["Status"]) ? $_REQUEST["Status"] : '';
		
		$pendingTrip = get_value('register_driver', 'pendingTripId', 'iDriverId',$Did,'','true');
		if($pendingTrip == NULL || $pendingTrip == ''){
    		$lastOnline  = @date("Y-m-d H:i:s");
    		$sql = "UPDATE `register_driver` SET tLastOnline= '$lastOnline',  vAvailability = '$availabilityStatus'   where iDriverId = $Did";
    		 $id1= $obj->sql_query($sql);
    		if($id1>0){
    				$returnArr['Action']="1";
            		$returnArr['message']='UpdateSuccessful';
            		echo json_encode($returnArr);
    			}else{
    			$returnArr['Action']="0";
	...
	if($type=="fetchPendingTrip"){
		
		$Did              = isset($_REQUEST["iDriverId"]) ? $_REQUEST["iDriverId"] : '';
		
		$sql    = "SELECT pendingTripId FROM `register_driver` WHERE iDriverId=$Did";
		$row = $obj->MySQLSelect($sql);
		
		$pendingTripId = $row[0]['pendingTripId'];
		if($pendingTripId != NULL && $pendingTripId != "")
		{
    		$availabilityStatus = 'Not Available';
    		$sql = "UPDATE `register_driver` SET vAvailability = '$availabilityStatus' , iTripId = '$pendingTripId', vTripStatus = 'Active', pendingTripId = NULL  where iDriverId = $Did";
		    $id1= $obj->sql_query($sql);
		    
		    if($id1>0){
        		$returnArr['Action']="1";
        		$returnArr['message']='Pending Trip Activated';
        		echo json_encode($returnArr);
		        
		    }else{
		        $returnArr['Action']="0";
        		$returnArr['message']='Trip Activation Failed';
        		echo json_encode($returnArr);
		    }
		    
		}else{
		    $returnArr['Action']="0";
    		$returnArr['message']='No Pending Trip For Driver';
    		echo json_encode($returnArr);
		}
	...
	if($type=="checkPendingTrip"){
		...
		{
    		$returnArr['Action']="1";
    		$returnArr['message']='Pending Trip Available for Driver';
    		echo json_encode($returnArr);
		...
		if($iDriverVehicleId > 0){
			...
			if(count($vCarType_Arr)>0){
				for($i=0;$i<count($vCarType_Arr);$i++){    
					...
					######### Checking For Flattrip #########
					$Fare_Single_Vehicle_Data=calculateFareEstimateAll($time,$seconds,$distance,$vCarType_Arr[$i]['iVehicleTypeId'],$driverId,1,"","","",1,0,0,0,"DisplySingleVehicleFare","Driver",1,"",$isDestinationAdded,$eFlatTrip,$fFlatTripPrice,$sourceLocationArr,$destinationLocationArr);
  ...
  #############################################################################
	...
	if($type == "UpdateDriverWorkLocationUFX") {
		...
		$vWorkLocation = isset($_REQUEST["vWorkLocation"]) ? $_REQUEST["vWorkLocation"] : '';
				...
				// $myfile = file_put_contents('WorkLocation.txt', PHP_EOL.'----------------------------------'.PHP_EOL.json_encode($_REQUEST) , FILE_APPEND | LOCK_EX);	
